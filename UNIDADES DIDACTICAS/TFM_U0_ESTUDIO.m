...
try
    ...
    go_to_position(0, idnum, demo, canal, gain_conversion, offset_conversion, gain_direction, bias_direction, bias_motor);
...
if delta_y < 0.1 % SI EL MOVIMIENTO ES INSIGNIFICANTE, NO SE PUEDE ANALIZAR
    ...
    % --- AHORA LOS CÁLCULOS SE HACEN SOBRE LOS DATOS "LIMPIOS" (UNIQUE_POS Y UNIQUE_TIME) ---
    ...
    % --- CÁLCULO DE PARÁMETROS DEL MODELO FOPDT (FIRST ORDER PLUS DEAD TIME) ---
...
function go_to_position(posicion_objetivo_cm, idnum, demo, canal, gain_conv, offset_conv, gain_dir, bias_dir, bias_mot)
    Kp_retorno = 0.8; 
    tolerancia_cm = 0.1;
    timeout_s = 20;
